...
const EXAMPLE_ROUTES = [
  {
    ...
    gradient: 'from-rally-peach/30 to-rally-butter/30',
    border: 'border-rally-peach/20',
  },
  {
    ...
    gradient: 'from-rally-lavender/30 to-rally-blush/30',
    border: 'border-rally-lavender/20',
  },
  {
    ...
    gradient: 'from-rally-butter/30 to-rally-powder/30',
    border: 'border-rally-butter/20',
  },
  {
    ...
    gradient: 'from-rally-blush/30 to-rally-lavender/30',
    border: 'border-rally-blush/20',
  },
];

const HOW_IT_WORKS = [
  { step: '01', title: 'Pick Your Vibe', desc: 'Choose a mood — cozy, chaotic, romantic, or whatever you\'re feeling.', icon: '🎯', accent: 'bg-rally-lavender/40' },
  { step: '02', title: 'Set Your Filters', desc: 'Budget, time, group, radius. Tell Rally what you\'re working with.', icon: '⚙️', accent: 'bg-rally-butter/40' },
  { step: '03', title: 'Get Your Route', desc: 'Rally generates a curated 2–4 stop outing with real nearby spots.', icon: '🗺️', accent: 'bg-rally-sage/40' },
  { step: '04', title: 'Go Live It', desc: 'Follow the route, reroll stops you don\'t like, share it with friends.', icon: '🎉', accent: 'bg-rally-peach/40' },
];

const HERO_LINES = [
  { text: 'Fun nights,', gradient: false },
  { text: 'less', gradient: false },
  { text: 'overthinking', gradient: true },
];
...
export default function HomePage() {
  return (
    <main className="relative overflow-x-hidden">
      ...
      {/* ── Background Orbs ──── */}
      <div className="bg-orb w-[500px] h-[500px] bg-rally-lavender -top-40 -left-40 fixed" />
      <div className="bg-orb w-[400px] h-[400px] bg-rally-blush -bottom-32 -right-32 fixed" />
      ...
      <section className="gradient-mesh relative px-5 pt-8 pb-12 sm:pt-12 sm:pb-20 md:pt-24 md:pb-32 max-w-6xl mx-auto text-center">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 mb-5 rounded-full bg-rally-500/10 border border-rally-500/20 text-rally-adaptive text-sm font-medium cursor-default"
        >
          <span className="w-2 h-2 rounded-full bg-rally-500 animate-pulse" />
        ...
        >
          Rally helps you turn &quot;I don&apos;t know what to do&quot; into a plan that actually sounds fun. Dates, hangouts, spontaneous nights — figured out.
        </motion.p>
        ...
        <motion.div
          ...
          className="mt-10 mx-auto max-w-xs glass-card animate-float p-4 bg-rally-lavender/15 text-left"
        >
          <div className="flex items-center gap-2 mb-3">
            <span className="w-2 h-2 rounded-full bg-rally-emerald animate-pulse shrink-0" />
            <span className="text-xs text-status-success font-medium">Live Route — Main Character Mode</span>
          </div>
          {[
            ...
            <div key={i} className="flex items-center gap-2.5 text-sm text-text-secondary py-1.5 border-b border-border-default last:border-0">
              <span className="w-5 h-5 rounded-full bg-linear-to-br from-rally-500/20 to-rally-pink/20 text-xs font-bold flex items-center justify-center shrink-0 text-rally-adaptive">
                {i + 1}
              </span>
              ...
              <motion.div key={feature.id} variants={cardReveal}>
                <motion.div whileHover={{ y: -3 }} transition={EASE_SPRING_SOFT}>
                  <Link href="/build" className={`block glass-card-hover p-5 sm:p-6 bg-linear-to-br ${feature.gradient} relative overflow-hidden glow-border`}>
                    <div className="absolute top-3 right-3 text-xs font-semibold px-2.5 py-1 rounded-full bg-surface-elevated text-text-secondary">
                      Tonight
                    </div>
                    <h3 className="text-base sm:text-lg font-bold mb-1.5 pr-16">{feature.title}</h3>
                    <p className="text-sm text-text-secondary leading-relaxed">{feature.subtitle}</p>
                  </Link>
                </motion.div>
              </motion.div>
            ))}
        ...
        >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
            {HOW_IT_WORKS.map((step) => (
              ...
              >
                <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl ${step.accent} flex items-center justify-center text-2xl sm:text-3xl mb-2 sm:mb-3 mx-auto`}>
                  {step.icon}
                </div>
                <div className="text-[10px] sm:text-xs font-bold text-rally-adaptive mb-1.5">STEP {step.step}</div>
              ...
              >
                ...
                <ol className="space-y-1.5 sm:space-y-2">
                  {route.stops.map((stop, j) => (
                    ...
                    >
                      <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-surface-elevated flex items-center justify-center text-[10px] sm:text-xs font-bold text-rally-adaptive shrink-0 mt-0.5">
                        {j + 1}
                      </span>
                      {stop}
                    </motion.li>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar -mx-5 px-5 snap-x snap-mandatory">
            {LOCAL_GEMS.map((gem, i) => (
              ...
              >
                ...
                <p className="text-xs text-rally-adaptive mb-1.5">{gem.neighborhood}</p>
      ...
      <section className="px-5 pb-12 sm:pb-20 max-w-6xl mx-auto">
        ...
        >
          <motion.div
            ...
            whileHover={{ y: -2 }}
            transition={EASE_SPRING_SOFT}
            className="glass-card glow-border p-6 sm:p-8 md:p-12 text-center gradient-date-night"
          >
            <motion.div
              animate={{ y: [0, -5, 0] }}
      ...
      <section className="px-5 pb-12 sm:pb-20 max-w-6xl mx-auto">
        ...
        >
          <motion.div
            variants={cardReveal}
            className="glass-card glow-border p-6 sm:p-8 md:p-12 bg-linear-to-br from-rally-sage/30 to-rally-powder/30 border-rally-sage/20"
          >
            <div className="md:flex items-center gap-8 lg:gap-12">
              <div className="flex-1 mb-6 md:mb-0">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rally-sage/30 border border-rally-sage/40 text-status-success text-xs font-medium mb-3">
